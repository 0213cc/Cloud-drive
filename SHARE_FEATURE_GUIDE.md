...
### 场景 7: Bob 更新文件内容（可写权限）

```powershell
# 1. 确保 Bob 有可写权限 (在场景2中已设置)
...
# 3. Bob 下载共享文件
python client.py download 2

# 4. Bob 在本地修改文件内容
# (手动在 downloads/notes.txt 文件中添加 "Bob's update")

# 5. Bob 上传新版本以更新文件
python client.py update 2 downloads/notes.txt

# 预期输出：
# ✓ 文件更新成功: notes.txt (新版本: 2)
```
...
当两个用户同时修改同一个文件时，系统会自动处理版本冲突，以确保不会有任何数据丢失。

**工作原理:**

1.  当你使用 `update` 命令上传文件时，客户端会记录下你所基于的文件的版本号。
2.  服务器会比较你提交的版本号和服务器上最新的版本号。
3.  **如果版本一致**：你的修改会成功覆盖，文件版本号加一。
4.  **如果版本不一致**（意味着在你编辑期间，有其他人已经上传了新版本）：
    - 服务器会**拒绝**直接覆盖，以保护他人的修改。
    - 你的修改会被保存为一个**新的冲突文件**，文件名类似于 `原文件名 (conflicted copy from 用户名 on 日期).txt`。
    - 原始文件保持不变。

**用户操作**：

- 当看到提示“Conflict detected...”时，你需要手动处理冲突。
- 下载原始文件和新生成的冲突文件。
- 手动对比两个文件，合并你需要的修改。
- 最后，将合并后的最终版本再次上传。

这个机制确保了所有人的工作都被保留，并给予用户最终的决定权来合并修改。
...
### Q7: 如果我和别人同时修改了同一个文件会怎么样？

系统会自动检测到这种情况并处理冲突，确保不会有数据丢失。

- 你的修改会被保存为一个新的**冲突文件**，例如 `myfile (conflicted copy from bob on 2024-11-17).txt`。
- 原始文件会保持另一位用户的最新版本。
- 你需要手动下载这两个文件，合并修改，然后再次上传最终版本。
...
# 更新文件内容（上传新版本）
python client.py update <file_id> <file_path>
